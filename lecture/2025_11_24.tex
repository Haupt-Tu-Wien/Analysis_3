\chapter{}
In dieser Vorlesung witmen wir uns der Initioalen Topologie. 
Um sie zu Motivierne betrachten wir zu nächst 
zwei kommutatuve Diagramme, die die Konstruktion der initialen 
Topologie veranschaulichen sollen.

\section{Kommutative Diagramme zur Veranschaulichung}

Weil die Konstrucktion bei einem normalen (Auch bei nicht normalen)
menschen
ein paar Knoten im kopf verursacht, wollen wir uns das ganze mal 
mit Bildern anschauen (Kommutative Diagramme).\\
Seien $X_i, i\in I$ Mengen "mit Haxen" und 
$
\Phi:
...
\node (Xk-1) at (-4,-4) {$(X_{k-1})$,};
\node (dots) at (-3,-4) {\quad$\dots$,};
\node (Xk) at (-2,-4) {$(X_{k})$,};
\node (X0) at (0,-4) {$\dots$};
\node (Xj) at (2,-4) {,$(X_{j})$,};
\node (dots2) at (3,-4) {$\dots$,};
\node (Xj+1) at (4,-4) {\quad ,$(X_{j+1})$};

% f_i, f_j
\draw[->, dashed] (Y) -- node[left] {$\exists!$} node[right] {$\Phi$} (Xi);
...
\dfn{}
{
    Seien $\langle X_i,\mathcal{T}_i \rangle, i \in I$ topologische Räume, $X$ eine Menge
    und $f_i: X \to X_i$ Abbildungen, $\forall i \in I$.
    Die \textbf{initiale Topologie} $\mathcal{T}_{\text{init}}$ auf $X$ von $\langle X_i,\mathcal{T}_i\rangle, i \in I$ 
    bezüglich der $f_i$ ist definiert als die kleinste Topologie die 
    $$ 
    \mathcal{T}_{\text{init}} \supseteq \bigcup_{i \in I}\{f_i^{-1}(O_i) \mid O_i \in \mathcal{T}_i\}
    $$
    erfüllt.
}
    Die Wohl interessanteste eigenschaft ist das 
$$\forall i \in I: f_i: \langle X, \mathcal{T}_{\text{init}} 
\rangle \to \langle X_i, \mathcal{T}_i \rangle$$ stetig ist. Das ist sogar
so ineressant das man sie auch oft wie folgt definiert:
\dfn{}
{
    Seien $\langle X_i,\mathcal{T}_i \rangle, i \in I$ topologische Räume, $i \in I$
    $X$ eine Menge und $f_i: X \to X_i$ Abbildungen, $\forall i \in I$.
    Die initiale \textbf{Topologie} $\mathcal{T_{\text{init}}}$ auf $X$ bezüglich der Familie von Abbildungen 
